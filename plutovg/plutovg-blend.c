        ...
        while(count--) {
                ...
                while(length) {
                        ...
                        while(b < end) {
                                ...
                                float real_x_end = round((x_tile + 1.0f) * texture->real_x_extent);
                                if(px >= real_x_end) {
                                        real_x_end = round((x_tile + 2.0f) * texture->real_x_extent);
                                        x_tile += 1;
                                }
                                float real_x_start = round(x_tile * texture->real_x_extent);
                                if(px < real_x_start) {
                                        real_x_end = real_x_start;
                                        real_x_start = round((x_tile - 1.0f) * texture->real_x_extent);
                                        x_tile -= 1;
                                }
                                bool is_large_x = (real_x_end - real_x_start) > small_x;

                                float real_y_end = round((y_tile + 1.0f) * texture->real_y_extent);
                                if(py >= real_y_end) {
                                        real_y_end = round((y_tile + 2.0f) * texture->real_y_extent);
                                        y_tile += 1;
                                }
                                float real_y_start = round(y_tile * texture->real_y_extent);
                                if(py < real_y_start) {
                                        real_y_end = real_y_start;
                                        real_y_start = round((y_tile - 1.0f) * texture->real_y_extent);
                                        y_tile -= 1;
                                }
